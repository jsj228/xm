...
class AuthGroup extends Model
{
	...
	static public function getUserGroup($uid)
	{
		...
		$user_groups = $this->field('uid,group_id,title,description,rules')->table($prefix . self::AUTH_GROUP_ACCESS . ' a')->join($prefix . self::AUTH_GROUP . ' g on a.group_id=g.id')->where(['a.uid' => $uid, 'g.status' => 1])->select();
	...
	static public function getAuthExtend($uid, $type, $session)
	{
		...
		$prefix = config('Db_PREFIX');
		$result = Db::table($prefix . self::AUTH_GROUP_ACCESS . ' g')->join($prefix . self::AUTH_EXTEND . ' c on g.group_id=c.group_id')->where(['g.uid' => $uid, 'c.type' => $type, 'extend_id' => ['exp', 'IS NOT NULL']])->getfield('extend_id', true);
	...
	static public function memberInGroup($group_id)
	{
		...
		$list = $this->field('m.uid,u.username,m.last_login_time,m.last_login_ip,m.status')->table($l_table . ' m')->join($r_table . ' a ON m.uid=a.uid')->join($r_table2 . ' u ON m.uid=u.id')->where(array('a.group_id' => $group_id))->select();
		return $list;
	}
