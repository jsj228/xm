...
class AdminCommon extends Controller
{
    ...
    {
        $this->redirect($this->request->module().'/Index/index');
    }
    ...
    {
        $id = array_unique((array)input('id'));
        $id = (is_array($id) ? implode(',', $id) : $id);
        $where = array_merge(array(
            'id' => array('in', $id)
        ), (array)$where);
        $msg = array_merge(array('success' => '操作成功！', 'error' => '操作失败！', 'url' => '', 'ajax' => $this->request->isAjax()), (array)$msg);
    ...
    final protected function accessControl()
    {
        ...
        $check = strtolower(request()->module() . '/' . request()->action());
        ...
        if (empty($menus)) {
            ...
            if ($current) {
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if (!ROOT_PATH && !$this->checkRule($item['url'], \Common\Model\AuthRuleModel::RULE_MAIN, null)) {
    ...
    final protected function returnNodes($tree = true)
    {
        ...
        if ((int)$tree) {
            $list = Db::name('Menu')->field('id,pid,title,url,tip,hide')->where(['hide'=>0])->order('sort asc')->select();
            ...
            $nodes = Db::name('Menu')->field('title,url,tip,pid')->where(['hide'=>0])->order('sort asc')->select();
    ...
    protected function lists($model, $where = array(), $order = '', $base = [ 'status' => ['egt', 0]], $field = true)
    {
        ...
        if (isset($REQUEST['r'])) {
            ...
            $listRows = (0 < config('LIST_ROWS') ? C('LIST_ROWS') : 10);
        }
