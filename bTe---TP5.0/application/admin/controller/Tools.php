...
class Tools extends AdminCommon
{
...
//        	$this->error('权限不足，您不能清理 Redis 缓存！');
    ...
    {
        $coin = input('coinname/s');//$_POST['coinname'];
        $fee = input('fee/f','0.006');//$_POST['fee'];
        $num = 0;
        if ($this->request->isPost()) {
            if ($coin) {
                $coinconf = DB::name('Coin')->where(['name' => trim($coin), 'status' => 1])->find();
                if (!$coinconf) {
                    $this->error('无效的币种!');
                }
...
//                                            'gas' => '0x76c0',
	...
	protected function rmdirr()
	{
        $dirname =strval(I('dirname'));
	...
	{
		die();
		redirect(request()->module().'/Tools/database?type=export');
	}

	public function dataImport()
	{
		die();
		redirect(request()->module().'/Tools/database?type=import');
	}
	...
	public function qianbao()
	{
		...
		$qb_list = Cache::store('redis')->get($s_key);

		if (!$qb_list) {
			$qb_list = DB::name('Coin')->where(['type' => ['in', 'bit,eth']])->select();
			Cache::store('redis')->set($s_key, $qb_list);
		}
