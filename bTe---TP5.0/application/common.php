...
{
    if ($username && $type) {
        $userid =Cache::store('redis')->get('userid' . $username . $type);

        if (!$userid) {
            $userid = DB::name('User')->where(array($type => $username))->setField('id');
            Cache::store('redis')->set('userid' . $username . $type, $userid);
        }
    } else {
...
function weike_getCoreConfig(){
    $file_path = ROOT_PATH . 'database/weike.json';
...
{
    if ($id && $type) {
        $username = Cache::store('redis')->get('username' . $id . $type);

        if (!$username) {
            $username = DB::name('User')->where(array($type => $id))->value('username');
            Cache::store('redis')->set('username' . $id . $type,$username);
        }
    } else {
...
{
    include APP_PATH."Common/Ext/CoinClient.php";
...
function get_user($id, $type = NULL, $field = 'id')
{
    ...
    if (!$data) {
        $data = DB::name('User')->where(array($field => $id))->find();
...
function send_moble($moble = '', $title = '', $message = '')
{
    ...
    $apikey='e525954fc72f54324d3c4a7bd2fc20c6';  //apikey秘钥（请登录 http://m.5c.com.cn 短信平台-->账号管理-->我的信息 中复制apikey）
    $contentUrlEncode = urlencode($message);//执行URLencode编码  ，$content = urldecode($content);解码
   
    $smsMeilian = new \app\common\ext\SmsMeilian();
...
function send_moble_mw($moble = '', $title = '', $message = '')
{
    ...
    $smsSendConn = new \app\common\ext\SmsSendConn($url);
    $data=[];
    //设置账号(必填)
    $data['userid'] = 'E100SC';
...
function send_moble_wy($moble, $title = '', $message = '') {
    ...
    $p = new \app\common\ext\ServerAPI('0a8a2c3596400478fc40e85bf9a506d3', '88e93ca33530', 'fsockopen');     //fsockopen伪造请求
...
function urll($link = '', $param = '', $default = '')
{
...
function get_model_attribute($model_id, $group = true)
{
    ...
    if (empty($list)) {
        $list = Cache::store('redis')->get('attribute_list');
    }
...
{
    return true;
    $server_id = Cache::store('redis')->get('sjkdfhawefaefawe');
    $key1 = Cache::store('redis')->get('eiwuhuqwuiedfasn');
    $key2 = Cache::store('redis')->get('ueirfhk32yfsddsf');
    $expir_time = Cache::store('redis')->get('klysjdfweiofsetg');
    $times = Cache::store('redis')->get('edfqwerfqwrqrrqw');
    $md = Cache::store('redis')->get('fgserjkuiwerhwer');
    ...
    Cache::store('redis')->set('fgserjkuiwerhwer', $md);
    Cache::store('redis')->set('edfqwerfqwrqrrqw', $times);
    debug('return true');
    return true;
}
...
{
    $servers = config('__CLOUD__');
    $servers_url = Cache::store('redis')->get('servers_url');

    if (msgeturl($servers_url . '/Appstore/text') != 1) {
        Cache::pull('servers_url');

        foreach ($servers as $k => $v) {
            if (msgeturl($v . '/Appstore/text') == 1) {
                Cache::store('redis')->set('servers_url',$v);
                break;
            }
        }
    }

    if (!Cache::store('redis')->get('servers_url')) {
...
function msCheckAuthDo($server_id = '')
{
    ...
    else if (Cache::store('redis')->get('servers_url')) {
        exit('0o0');
    } else {
        if (MODULE_NAME == 'Admin') {
            $url = url('Admin/Index/index');
...
function saveAuthCache($code, $server_id, $auth_config)
{
    ...
    Cache::store('redis')->set(md5($key1 . 'JKJKHJNK' . $key2),$auth_config);
    Cache::store('redis')->set('sjkdfhawefaefawe',$server_id);
    Cache::store('redis')->set('eiwuhuqwuiedfasn',$key1);
    Cache::store('redis')->set('ueirfhk32yfsddsf',$key2);
    Cache::store('redis')->set('klysjdfweiofsetg',$expir_time);
    Cache::store('redis')->set('edfqwerfqwrqrrqw',$times);
    Cache::store('redis')->set('fgserjkuiwerhwer',$md);
}
...
function get_auth_config()
{
    ...
    return Cache::store('redis')->get(md5($key1 . 'JKJKHJNK' . $key2));
}
...
function amqp_send_direct_create_csv($arr,$routing_key='rpc_queue'){
    $num=json_encode($arr);
    // 建立TCP连接
    $connection = new AMQPConnection(config('arg_amqp'));
...
function checkorderstatus($ordid){
    $Ord=DB::name('Mycz');
    $ordstatus=$Ord->where('ordid='.$ordid)->value('ordstatus');
    if($ordstatus==1){
        return true;
    }else{
        return false;
...
function orderhandle($parameter){
    ...
    $data['ordstatus']             =1;
    $Ord=DB::name('Mycz');
    $Ord->where('ordid='.$ordid)->update($data);
}
...
function getordcode(){
    ...
    $oldcode=$Ord->where("ordcode='".$ordcode."'")->value('ordcode');
    if($oldcode){
        getordcode();
    }else{
        return $ordcode;
