...
use think\Cache;
use think\sessoin;
class Issue extends HomeCommon
{
	public function index()
	{
		...
		$where['status'] = ['neq', 0];
		$Model = Db::name('Issue');
        $list = $Model->where($where)->order('tuijian asc,paixu desc,addtime desc')->paginate(10,false,[]);
        $page = $list->render();
        $tuijian = $Model->where(array("tuijian"=>1))->order("addtime desc")->limit(1)->find();

		if($tuijian){
			$tuijian['coinname'] = config::get('coin')[$tuijian['coinname']]['title'];
			$tuijian['buycoin']  = config::get('coin')[$tuijian['buycoin']]['title'];
	...
	public function log()
	{
		...
		$page = $list->render();
		$list = $IssueLog->where($where)->order('id desc')->paginate(10, false,[]);

		if ($list){
            foreach ($list as $k => $v) {
                $data = $v;
                $data['shen'] = round((($v['ci'] - $v['unlock']) * $v['num']) / $v['ci'], 6);
	...
	public function upbuy()
	{
		...
		$rs = array();
		$finance =Db::table('weike_finance')->where(array('userid' => userid()))->order('id desc')->find();
		$finance_num_user_coin = Db::table('weike_user_coin')->where(array('userid' => userid()))->find();
		$rs[] = Db::table('weike_user_coin')->where(array('userid' => userid()))->setDec($Issue['buycoin'], $mum);
		$rs[] = $finance_nameid = Db::table('weike_issue_log')->add(array('userid' => userid(), 'coinname' => $Issue['coinname'], 'buycoin' => $Issue['buycoin'], 'name' => $Issue['name'], 'price' => $Issue['price'], 'num' => $num, 'mum' => $mum, 'ci' => $Issue['ci'], 'jian' => $Issue['jian'], 'unlock' => 0, 'addtime' => time(), 'endtime' => $tm, 'status' => 0));
	...
	public function unlock()
	{
		...
		$rs[] = Db::table('weike_issue_log')->where(array('id' => $IssueLog['id']))->save(array('unlock' => $IssueLog['unlock'] + 1, 'endtime' => $tm));

		if ($IssueLog['ci'] <= $IssueLog['unlock'] + 1) {
			$rs[] = Db::table('weike_issue_log')->where(array('id' => $IssueLog['id']))->save(array('status' => 1));
		}
