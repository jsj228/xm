...
class BuilderList extends Admin
{
	...
	public function doDeleteTrue($ids)
	{
	    $model = input("model");
	    $ids = input('ids');
		$ids = (is_array($ids) ? $ids : explode(',', $ids));
		Db::name($model)->where(array(
			'id' => array('in', $ids)
			))->delete();
	...
	public function display($templateFile = '', $charset = '', $contentType = '', $content = '', $prefix = '')
	{
			...
			foreach ($actions as $action) {
				...
				if (isset($action['opt'])) {
					...
					if (count($linkTextArr) == 1) {
						...
						if (isset($item[$linkTextArr[2]]) && $item[$linkTextArr[2]] ) {
							$result[] = '';
						}
						else {
        ...
        $pager = \think\paginator\driver\Bootstrap::make($this->_data,$this->_pagination['listRows'],$this->_pagination['parameter']['p'],
            $this->_pagination['totalCount'],false,$this->_pagination['parameter']);

		$paginationHtml = $pager->render();
		...
		return parent::fetch('public/list');
	}
}
