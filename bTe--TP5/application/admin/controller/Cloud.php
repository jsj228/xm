...
class Cloud extends Admin
{
    ...
    {
        $this->redirect(__MODULE__."/cloud/theme");
        exit;
        return $this->fetch();
    }
    ...
    public function kefu()
    {
        ...
        if (!in_array('kefu', $DbFields)) {
            Db::execute('ALTER TABLE `weike_config` ADD COLUMN `kefu` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
        }
    ...
    public function theme()
    {
        ...
        if (!in_array('index_html', $DbFields)) {
            Db::execute('ALTER TABLE `weike_config` ADD COLUMN `index_html` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
        }
    ...
    public function getFileList()
    {
        ...
        session('currentVersion',$currentVersion);
    }
    ...
    {
        if ((cache('CLOUDTIME') + (60 * 60)) < time()) {
            cache('CLOUD', null);
            cache('CLOUD_IP', null);
            cache('CLOUD_HOME', null);
            cache('CLOUD_DAOQI', null);
            cache('CLOUD_GAME', null);
            cache('CLOUDTIME', time());
        }

        $CLOUD = cache('CLOUD');
        $CLOUD_IP = cache('CLOUD_IP');
        $CLOUD_HOME = cache('CLOUD_HOME');
        $CLOUD_DAOQI = cache('CLOUD_DAOQI');
