...
class CloudController extends AdminController
{
    ...
    {
        redirect(__MODULE__."/Cloud/theme.html");
        exit;
        $this->display();
    }
    ...
    public function kefu()
    {
        ...
        if (!in_array('kefu', $DbFields)) {
            M()->execute('ALTER TABLE `weike_config` ADD COLUMN `kefu` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
        }
    ...
    public function theme()
    {
        ...
        if (!in_array('index_html', $DbFields)) {
            M()->execute('ALTER TABLE `weike_config` ADD COLUMN `index_html` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
        }
