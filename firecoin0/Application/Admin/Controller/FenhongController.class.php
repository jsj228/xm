...
class FenhongController extends AdminController
{
	public function index()
	{
        $p = I('p',1);
        $r = I('r',15);
        $str_addtime = I('str_addtime');
        $end_addtime =I('end_addtime');
        $order = I('order');
        $status = I('status');
        $field = I('field');
        $type = I('type');
	...
	public function fenfa()
	{
		...
		if ((0 < $id) && ($id <= S('fenhong_fenfa_c'))) {
			...
			$mo->execute('lock tables weike_user_coin write,weike_fenhong_log write');
			$rs = array();
			$rs[] = $mo->table('weike_user_coin')->where(array('userid' => $fenfa[$id]['userid']))->setInc($fenhong['coinjian'], $faduoshao);
			$rs[] = $mo->table('weike_fenhong_log')->add(array('name' => $fenhong['name'], 'userid' => $fenfa[$id]['userid'], 'coinname' => $fenhong['coinname'], 'coinjian' => $fenhong['coinjian'], 'fenzong' => $fenhong['num'], 'price' => $dange, 'num' => $fenfa[$id]['chiyou'], 'mum' => $faduoshao, 'addtime' => time(), 'status' => 1));
	...
	public  function usercny(){
		...
		if ($field && $name) {
			if ($field == 'username') {
				$where['f.userid'] = M('User')->where(array('username' => $name))->getField('id');
			} else {
				$where[$field] = $name;
			}
		}
		$where['f.release'] =1;
		$count = count( M('FenhongLog f')
			->join('left join weike_user as u on u.id = f.userid')
			->where($where)
			->select());//记录总数
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['adminname'] = M('Admin')->where(array('id' =>session('admin_id')))->getField('username');
		}
