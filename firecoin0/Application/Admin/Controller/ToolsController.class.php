...
//        	$this->error('权限不足，您不能清理 Redis 缓存！');
    ...
    {
        $coin = I('coinname/s');//$_POST['coinname'];
        $fee = I('fee/f','0.006');//$_POST['fee'];
        $num = 0;
        if (IS_POST) {
            if ($coin) {
                $coinconf = M('Coin')->where(['name' => trim($coin), 'status' => 1])->find();
                if (!$coinconf) {
                    $this->error('无效的币种!');
                }
                ...
                if ($coinconf['type'] == 'eth' || $coinconf['type'] == 'token') {
                    ...
                    $coin_sum = 0;
                    $id = 0;
                    $accounts = M('UserCoin')->where([$coinb => ['neq', ''], $coinp => ['neq', '']])->select();
                    //筛选钱包中钱包中大于 0.01 的用户
                    if (count($accounts) > 0) {
...
//                                            'gas' => '0x76c0',
	...
	public function qianbao()
	{
		...
		if (!$qb_list) {
			$qb_list = M('Coin')->where(['type' => ['in', 'bit,eth']])->select();
			S($s_key, $qb_list);
		}
