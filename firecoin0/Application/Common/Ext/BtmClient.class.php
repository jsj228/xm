        ...
        if ($res['status'] == 'success' || $res['status'] == 'SUCCESS'){
            ...
            foreach ($res_cut as $k=>$v){
                ...
                foreach ($v['outputs'] as $out_k=>$out_v){
                    if (in_array($out_v['address'],$addList)){
                        if (preg_match('/([1-9]|[a-z]|[A-Z])/',$v['block_hash']) && $v['block_height'] && $v['block_index']){
                            $zrData = M('myzr')->where('username="'.$out_v['address'].'" and coinname="btm" and txid="'.$v['tx_id'].'"')->find();
                            if ($zrData){
                                //数据已经存在
                                if ($zrData['status'] == 1){
                                    continue;
                                    ...
                                    M()->startTrans();
                                    $zrUp = M('myzr')->where('id='.$zrData['id'])->save(['status'=>1]);
                                    $userUp = M('user_coin')->where('userid='.$zrData['userid'])->setInc('btm',$out_v['amount']/100000000);
                                    if (!$zrUp || !$userUp){
                                        M()->rollback();
                                    }else{
                                        M()->commit();
                                ...
                                //数据库不存在
                                $uid = M('user_coin')->where('btmzb="'.$out_v['address'].'"')->getField('userid');
                                if ($uid){
                                    $zrUpData = [
                                        'userid'        =>  $uid,
                                        'username'      =>  $out_v['address'],
                                        'coinname'      =>  'btm',
                                        'txid'          =>  $v['tx_id'],
                                        'num'           =>  $out_v['amount']/100000000,
                                        'mum'           =>  $out_v['amount']/100000000,
                                        'addtime'       =>  time(),
                                        'status'        =>  1
                                    ];
                                    M()->startTrans();
                                    $zrUp = M('myzr')->add($zrUpData);
                                    $userUp = M('user_coin')->where('userid='.$uid)->setInc('btm',$out_v['amount']/100000000);
                                    if (!$zrUp || !$userUp){
                                        M()->rollback();
                                    }else{
                                        M()->commit();
