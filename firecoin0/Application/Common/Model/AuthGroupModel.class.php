...
class AuthGroupModel extends \Think\Model
{
	...
	static public function getUserGroup($uid)
	{
		...
		$prefix = C('DB_PREFIX');
		$user_groups = M()->field('uid,group_id,title,description,rules')->table($prefix . self::AUTH_GROUP_ACCESS . ' a')->join($prefix . self::AUTH_GROUP . ' g on a.group_id=g.id')->where(['a.uid' => $uid, 'g.status' => 1])->select();
	...
	static public function getAuthExtend($uid, $type, $session)
	{
		...
		$prefix = C('DB_PREFIX');
		$result = M()->table($prefix . self::AUTH_GROUP_ACCESS . ' g')->join($prefix . self::AUTH_EXTEND . ' c on g.group_id=c.group_id')->where(['g.uid' => $uid, 'c.type' => $type, 'extend_id' => ['exp', 'IS NOT NULL']])->getfield('extend_id', true);
