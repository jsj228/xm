...
class TradeModel extends \Think\Model
{
	...
	public function moni($market = NULL)
	{
		...
		if ($type == 1) {
			...
			$rs[] = $mo->table('weike_trade')->add(array('userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		} else {
			$mo->execute('rollback');
...
//						M('TradeJson')->add(array('market' => $market, 'data' => '', 'addtime' => $na, 'type' => $v));
    ...
    public function chexiao($id = NULL)
    {
        ...
        if ($trade['type'] == 1) {
            $mo->startTrans();
            $trade = M('Trade')->lock(true)->where(array('id' => $id))->find();
            if ($trade['status'] != 0) {
                $mo->rollback();
                return array('0', '订单不能撤销');
            }
            try{
                ...
                if ($mun <= round($user_buy[$rmb . 'd'], 8)) {
                    ...
                    $save_buy_rmb = $user_buy[$rmb . 'd'];
                    /*if (in_array($trade['userid'],[14590,10802,11567,13341,17953,12096,7821,10121,11537,15580])){
                        $save_buy_rmb = $user_buy[$rmb . 'd'];
                    }else{
...
//				case 1:
//					$obj = new BuyRedis(C('redis'),$mysql_data['market']);
//					break;
//				case 2:
//					$obj = new SellRedis(C('redis'),$mysql_data['market']);
//					break;
//			}
//			$obj->init();
//			self::check_redis($id);
