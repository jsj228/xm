...
class UserModel extends \Think\Model
{
	...
	{
		$check_update_user = S('check_update_user');

		if (!$check_update_user) {
			$User_DbFields = M('User')->getDbFields();

			if (!in_array('alipay', $User_DbFields)) {
				M()->execute('ALTER TABLE `weike_user` ADD COLUMN `alipay` VARCHAR(200) NULL  COMMENT \'支付宝\' AFTER `status`;');
			}

			if (!in_array('email', $User_DbFields)) {
				M()->execute('ALTER TABLE `weike_user` ADD COLUMN `email` VARCHAR(200) NULL  COMMENT \'邮箱\' AFTER `status`;');
			}
	...
	public function get_userid($username = NULL)
	{
		...
		$get_userid_user = S('get_userid_user' . $username);
