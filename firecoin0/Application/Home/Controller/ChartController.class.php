...
//				$data[1] = $this->getTradeSell($market);
	...
	public function getMarketSpecialtyJson()
	{
		...
		if (!$tradeJson) {
			$tradeJson = M('TradeJson')->where(array('market' => $market, 'type'   => $time, 'data'   => array('neq', '')))->order('id asc')->select();
	...
	public function getSpecialtyTrades()
	{
		...
		if (!$input['since']) {
			...
			$tradeLog = M('TradeLog')->where(array('market' => $input['market'], 'id' => array('gt', $input['since'])))->order('id desc')->select();
			$json_data = array();
...
//
		...
		if (in_array($input['step'] / 60, $timearr)) {
			$time = $input['step'] / 60;
			$tradeJsonTime=M('TradeJson')->where(array('market' => $input['market'],'type'=>$time))->order('id desc')->getField('addtime');
			$btime=$tradeJsonTime;
			$etime=$tradeJsonTime+$input['step'];
			if(!$input['since']){
				$tradeLog = M('TradeJsonLog')->where(array('market' => $input['market']))->order('id desc')->select();
