...
class IndexController extends HomeController
{
	public function index()
	{
		...
		if (!$indexAdver) {
			$indexAdver = M('Adver')->where(array('status' => 1))->order('sort desc,id asc')->select();
					...
					foreach ($indexArticleType as $k => $v) {
						...
						foreach($indexArticle[$k] as $kk =>$vv){
							...
							if($indexArticle[$k][$kk]['img']){
								$indexArticle[$k][$kk]['img'] = C('TMPL_PARSE_STRING.__DOMAIN__') . "/Upload/article/".$indexArticle[$k][$kk]['img'];
							}else{
