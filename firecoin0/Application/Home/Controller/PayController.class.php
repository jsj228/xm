...
class PayController extends HomeController
{
	public function index()
	{
		if (IS_POST) {
			if (isset($_POST['alipay'])) {
				...
				$rs[] = $mo->table('weike_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function movepay()
	{
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables weike_user_coin write,weike_mycz write');
			$rs = array();
			$rs[] = $mo->table('weike_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
			$rs[] = $mo->table('weike_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function mycz()
	{
		...
		if (check($id, 'd')) {
			...
			$myczType = M('MyczType')->where(array('id' => $mycz['bank_id']))->find();
