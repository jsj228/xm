...
//			$this->error(['msg' => "error"]);
		...
		$path = 'Upload/idcard/';
		$filename = md5($_FILES['upload_file0']['name']) . uniqid() . userid() . '.' . $ext;
	...
	public function nameauth()
	{
		...
		if($user['idcardimg1']){
			...
			foreach($img_arr as $k=>$v){
				$imgstr .='<li><ul style="position: relative;left: -210px;width: 150%;height: auto;"><li style="float: left;margin-left: 2px;"><img style="width:280px;height:180px;" src="'.C('TMPL_PARSE_STRING.__DOMAIN__').'/Upload/idcard/'.$v.'" /></li></ul></li>';
				$imgnum++;
			}
...
//		dump($user);die;
	...
	{
        $oldpassword = I('oldpassword/s');
        $newpassword = I('newpassword/s');
	...
	{
        $oldpaypassword = I('oldpaypassword/s');
        $newpaypassword = I('newpaypassword/s');
	...
	{
        $moble_new = I('moble_new/d');
        $moble_verify_new = I('moble_verify_new/d');
	...
	{
        $alipay = I('alipay/d', NULL);
        $paypassword = I('paypassword/s', NULL);
	...
	{
        $paypassword = I('paypassword/s', NULL);
        $tpwdsetting = I('tpwdsetting/d', NULL);
	...
	public function upqianbao()
	{
		...
		$addr = I('addr/s');
		$code = I('code');
		$paypassword = I('payPWD/s');

       
		if (!userid()) {
			redirect('/#login');
		}
		if ($code != session('WeChatcode')) {
            $this->error('短信验证码错误！');
        }
		if (!check($name, 'a')) {
			$this->error('备注名称格式错误！');
		...
		if (M('UserQianbao')->add(array('userid' => userid(), 'name' => $name, 'addr' => $addr,'memo'=>$memo,'coinname' => $coin, 'addtime' => time(), 'status' => 1))) {
			$this->success('添加成功！');
		} else {
			$this->error('添加失败！');
		}
