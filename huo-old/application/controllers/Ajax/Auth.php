...
class Ajax_AuthController extends Ajax_BaseController
{
    ...
    public function sendregmsgAction()
    {
        ...
        {
            //短信过于频繁，请使用语音验证码
            $this->ajax($GLOBALS['MSG']['SMS_TO_VOICE_WARN'],0);
        }
...
//            $handkeep=$tMO->uploadOne($postData['basesan'],$url='');
            ...
            if((!$frontFace&&$postData['baseyi']) || (!$frontFace&&$postData['baseer'])  ||  (!$handkeep&&$postData['baseer']))
            {
                $this->ajax($tMO->getError(),0);
            }
            //print_r($_FILES);die;
    ...
    public function short_urlAction(){
        $url = "http://www.local.dobitradecode.com/?regfrom=yp979yp77bx&alert=register";
