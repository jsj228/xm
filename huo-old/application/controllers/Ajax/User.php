...
//        }
    ...
    public function loginAction()
    {
        ...
        //跳转URL安全检查
        $_COOKIE['reurl'] = !preg_match('#[\r\n]+|^http[s]?://(?![^?/]+?\.dobitrade.com).*$#i', $_COOKIE['reurl'])?:'/';
    ...
    public function coinOutAction()
    {
        ...
        if (!$user['pwdtrade'])
        {
            $this->ajax($GLOBALS['MSG']['NEED_REAL_AUTH'], 0, 'pwdtrade');
        }
        ...
        if (!$realInfo)
        {
            $this->ajax($GLOBALS['MSG']['NEED_REAL_AUTH']);
        }
        ...
        if ($cancoinout == 0)
        {
            $this->ajax($GLOBALS['MSG']['COIN_OUT_FROZEN']);
        }

        //钱包地址错误
        if (!$wallet)
        {
            $this->ajax($GLOBALS['MSG']['WALLET_ADDR_ERROR'], 0, 'wallet');
        }
         if(!$label  && $coinName=='eos')
         {
             $this->ajax($GLOBALS['MSG']['BIAOQIAN_NO'], 0, 'label');
         }
        //币信息
        ...
        {
            $this->ajax($GLOBALS['MSG']['COIN_OUT_STOP']);
        }
        ...
        {
            $this->ajax($GLOBALS['MSG']['NUMBER_ERROR'], 0, 'number');
        }

        //用户余额不足
        if (Tool_Math::comp($tNum, $this->mCurUser[$coinName . '_over'])==1)
        {
            $this->ajax($GLOBALS['MSG']['COIN_NOT_ENOUGH'], 0, 'number');
        }
        ...
        {
            $this->ajax($GLOBALS['MSG']['TRADE_PWD_ERROR'], 0, 'pwdtrade');
        }
        ...
        $tMO = new $mo();
        if (!$tMO->post($wallet, $tNum, $coinName, $this->mCurUser,$label))
        {
...
//            }
    ...
    public function smsnocapAction()
    {
        ...
        if(!$user['mo'])
        {
            $this->ajax($GLOBALS['MSG']['NEED_PHONE']);
        }
        $redis = Cache_Redis::instance();
        $count = $redis->get($key);
        ...
        {
            //短信过于频繁，请使用语音验证码
            $this->ajax($GLOBALS['MSG']['SMS_TO_VOICE_WARN']);
        }
...
//充币
    ...
    public function getCoinAddressAction()
    {
        ...
        //RGB系列
        if($cointype == 'rgb') $wallet = $addressMo->getAddrRgb($this->mCurUser['uid'], $name);
...
//        }
    ...
    public function inviteAction()
    {
        ...
        {
            case 1:
                //$this->getInviteRecordAction();
                //$this->get_AIT_InviteRecordAction();
                $this->get_MCC_InviteRecordAction();
                break;
            case 2:
                $this->getRebateRecordAction('out');
                break;
            case 3:
                $this->getRebateRecordAction('in');
                break;
            case 4:
                //$this->getInviteListAction();
                $this->get_ait_InviteListAction();
                break;
     ...
     * AIT 邀请记录
     */
    public function get_AIT_InviteRecordAction()
    {
        $coin='ait';//根据活动币种改币名
        $this->_ajax_islogin();
        $mo=$_SESSION['user']['mo'];
        ...
        $pagenumber=1000;
        $p= $pagenumber*($page-1);
        $activity = ActivityModel::getInstance()->where("name='赠送$coin'")->fRow();
        $starttime = $activity['start_time'];
        $endtime = $activity['end_time'];
        $sql="select c.*,d.number,d.coin,e.name Inviter_name,e.status Inviter_status,f.name be_invited_name,f.status be_invited_status,f.updated be_invited_updated from
...
left join user b on a.from_uid=b.uid where a.created>$starttime and a.created<$endtime and a.from_uid!=0) as c left join user_reward d on c.be_invited_uid=d.be_invited and (d.type=3 and d.coin='$coin')
left join autonym e on c.Inviter_uid=e.uid left join autonym f on c.be_invited_uid=f.uid where 1=1 and c.Inviter_mo=$mo limit $p,$pagenumber";
        $list=$usermo->query($sql);
        $data=array();
        foreach ($list as &$v) {
            $data[] = array(
                'phone' => preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1****$3', $v['be_invited_mo']),
                'date' => date('Y-m-d H:i:s', $v['be_invited_created']),
                'status' => array(
                    'code' => isset($v['be_invited_status'])&& $v['be_invited_status']==2 ? 1 : 0,
                    'txt' => isset($v['be_invited_status']) && $v['be_invited_status'] == 2 ? $GLOBALS['MSG']['REAL_AUTHED'] : $GLOBALS['MSG']['UNREAL_AUTHED']
                ),
                'reward' => $v['number']? trim(preg_replace('/(\.\d*?)0+$/', '$1', $v['number']), '.').strtoupper($coin):'',
            );
        }
        $this->ajax('', 1, $data);
    }
     ...
     * mcc邀请记录
     */
    public function get_MCC_InviteRecordAction()
    {
        $this->_ajax_islogin();
        $page = $_POST['page'] ?: 1;
        $coin='mcc';
        $activity = ActivityModel::getInstance()->where("name='赠送$coin' and admin=110")->fRow();
        $starttime = $activity['start_time'];
        $endtime = $activity['end_time'];
        $xUid = UserModel::getInstance()->field('uid')->where("from_uid={$this->mCurUser['uid']} and created>$starttime and created<$endtime")->fList();

        $data = array();
        if ($xUid) {
            $xUid = array_column($xUid, 'uid');
            $where = sprintf(' uid in (%s)', implode(',', $xUid));
            //用户信息
            $xUser = UserModel::getInstance()->field('mo,email,uid,created')->where($where)->order('uid desc')->page($page, 30)->fList();

            //实名状态
            $xUserRealInfo = AutonymModel::getInstance()->field('uid')->where($where . ' and status=2')->fList();

            if ($xUserRealInfo) {
                $xUserRealInfo = array_column($xUserRealInfo, 'uid', 'uid');
            }

            $rewardList = User_RebateLogModel::getInstance()
                ->field('coin, number, be_invited')
                ->where(['uid' => $this->mCurUser['uid'], 'be_invited' => array('>', 0)])
                ->fList();

            $reward = array();
            foreach ($rewardList as $v) {
                $reward[$v['be_invited']] = $v;
            }
            unset($v);
            ...
            foreach ($xUser as &$v) {
                ...
                $account= $v['mo']? preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1****$3', $v['mo']): $email;
                $data[] = array(
                    'phone' => $account,
                    //'phone' => preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1****$3', $v['mo']),
                    'date' => date('Y-m-d H:i:s', $v['created']),
                    'status' => array(
                        'code' => isset($xUserRealInfo[$v['uid']]) ? 1 : 0,
                        'txt' => isset($xUserRealInfo[$v['uid']]) ? $GLOBALS['MSG']['REAL_AUTHED'] : $GLOBALS['MSG']['UNREAL_AUTHED']
                    ),
                   // 'reward' => Tool_Math::eftnum($reward[$v['uid']]['number']) . strtoupper($reward[$v['uid']]['coin']),
                    'reward' => "",
                );
            }
        }
     ...
     * 邀请记录
     */
    public function getInviteRecordAction()
    {
        $this->_ajax_islogin();
        $page = $_POST['page']?:1;
        $xUid = UserModel::getInstance()->field('uid')->where(array('from_uid'=>$this->mCurUser['uid']))->fList();
        ...
        if($xUid)
        {
            ...
            $where = sprintf(' uid in (%s)', implode(',', $xUid));
            //用户信息
            $xUser = UserModel::getInstance()->field('mo,uid,created')->where($where)->order('uid desc')->page($page, 30)->fList();
     ...
     * 奖励提币记录
     */
    public function getRebateRecordAction($type='in')
    {
        $this->_ajax_islogin();
        $page = $_POST['page']?:1;
        $type = $type?:$_POST['type'];
        $where = array('uid'=>$this->mCurUser['uid'], 'type'=>$type=='in'?1:0);
        if($type=='in')
        {
            $where['be_invited'] = 0;
        }
        $data  = User_RebateLogModel::getInstance()->where($where)->page($page, 30)->order('id desc')->fList();
        ...
        //     foreach ($data as $v)
        //     {
        //         $exchangeId[$v['coin']][] = $v['exchange_id'];
        //     }

        //     $exchange = array();
        //     foreach($exchangeId as $k=>$v)
        //     {
        //         $mo = 'Exchange_'.ucfirst($k).'Model';
        //         $exData = $mo::getInstance()->where(sprintf('id in (%s)', implode(',', $v)))->fList();
        //         if($exData)
        //         {
        //             foreach($exData as $exOne)
        //             {
        //                 $exchange[$k.$exOne['id']] = $exOne;
        //             }
        //         }
        //     }
        // }
        ...
        foreach ($data as &$v)
        {
            $v['number'] = sprintf('%.8f', $v['number']);
            $v['created'] = date('Y-m-d H:i:s', $v['created']);
            $v['coin'] = strtoupper($v['coin']);
            unset($v['type'],$v['be_invited'],$v['exchange_id'],$v['uid']);
        }
        $this->ajax('', 1, $data);
    }
     ...
     * ait 邀请排行榜
     */
    public function get_ait_InviteListAction()
    {
        $coin = 'mcc';//根据活动币种改币名
        $user_rewordmo = new User_RewardModel();
        $activity = ActivityModel::getInstance()->where("name='赠送$coin' and admin=110")->fRow();
        $starttime = $activity['start_time'];
        $endtime = $activity['end_time'];
        $sql = "select * from (
select Inviter_uid,Inviter_mo,Inviter_email,count(be_invited_uid) uidtotal,count(status) autototal from (
 select a.from_uid Inviter_uid,b.mo Inviter_mo,b.area Inviter_area,b.email Inviter_email,a.uid be_invited_uid,a.mo be_invited_mo
...
where a.created>$starttime and a.created<$endtime and a.from_uid!=0) as g group by g.Inviter_uid ) as h order by h.autototal desc,h.uidtotal desc,h.Inviter_uid asc limit 200";
 ...
 left join autonym e on c.Inviter_uid=e.uid left join autonym f on c.be_invited_uid=f.uid) as g GROUP BY g.inviter_mo) as h order by number desc,invited_total desc,Inviter_uid asc limit 200";
        */
        $list = $user_rewordmo->query($sql);
        $data = array();
        /*  foreach($list as $k=>&$v){
              $data[$k]['invite_count']= $v['invited_total'];
              $data[$k]['mo'] = preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1****$3', $v['inviter_mo']);;
              $data[$k]['no'] = $k+1;
              $data[$k]['reward'] = $v['number']?trim(preg_replace('/(\.\d*?)0+$/', '$1', $v['number']), '.'). strtoupper($coin):'';
              $data[$k]['realname_number'] = $v['number'] ? $v['number']/15:0;//实名人数
          }*/
        foreach ($list as $k => &$v) {
            $data[$k]['invite_count'] = $v['uidtotal'];
            if ($v['Inviter_email']) {
                $email_array = explode("@", $v['Inviter_email']);
                $not = substr_replace($email_array[0], '****', 3);
                $email = $not . '@' . $email_array[1];
            }
            $account = $v['Inviter_mo'] ? preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1****$3', $v['Inviter_mo']) : $email;
            $data[$k]['mo'] = $account;
            $data[$k]['no'] = $k + 1;
           // $data[$k]['reward'] = $v['autototal'] ? trim(preg_replace('/(\.\d*?)0+$/', '$1', bcmul($v['autototal'], 15, 20)), '.') . strtoupper($coin) : '';
            $data[$k]['reward'] = '';
            $data[$k]['realname_number'] = $v['autototal'];//实名人数
        }
        ...
        $top5 = array(
            ...
            'cell****@gmail.com'=>[16,5],
            'andr****@gmail.com'=>[22,11],
            '185****3312'=>[3,1],
            '131****3734'=>[6,1],
            '180****9357'=>[8,4],
        );
        $cKey = 'InviteListFakeCache';
        $cache = Cache_Redis::instance()->get($cKey);
        if(($cache && ($cache = json_decode($cache, true)) && time()-$cache['updateTime']>rand(3600*3, 3600*5))||!$cache)
        {
            $top5 = $cache?$cache['data']:$top5;
            foreach($top5 as $k=>&$v)
            {
                $newNum = rand(5, 10);
                $v[0] += $newNum;
                $v[1] += rand(5, $newNum);
                $data[] = array('invite_count'=>$v[0], 'realname_number'=>$v[1], 'mo'=>$k, 'reward'=>'');
            }
            unset($v);

            $cacheData = array('data'=>$top5, 'updateTime'=>time());
            Cache_Redis::instance()->set($cKey, json_encode($cacheData));
        ...
        //排序
        array_multisort(array_column($data, 'realname_number'), SORT_DESC, array_column($data, 'invite_count'), SORT_DESC, $data);
        $no = 1;
        foreach ($data as &$v)
        {
            $v['no'] = $no++;
        ...
        $cacheData = array('data'=>$data, 'updateTime'=>time());
        Cache_Redis::instance()->set('InviteListCache', json_encode($cacheData));
        //$data and array_multisort(array_column($data, 'realname_number'), SORT_DESC, $data);
        $this->ajax('', 1, $data);
    }
    /**
     * 邀请排行榜
    ...
    public function getInviteListAction()
    {
        $this->_ajax_islogin();
        //邀请好友统计
        $inviteUser = UserModel::getInstance()->query('SELECT count(uid) count, from_uid FROM user WHERE from_uid>0 GROUP BY from_uid ORDER BY count DESC limit 20');
        $data = array();
        if($inviteUser)
        {
            $inviteUser = array_column($inviteUser, 'count', 'from_uid');
            $where = sprintf(' uid in (%s)', implode(',', array_keys($inviteUser)));
            $userData = UserModel::getInstance()->field('uid, rebate, mo')->where($where)->fList();
            $inviteRewardList = User_RebateLogModel::getInstance()->field('sum(number) number, uid, coin')->where($where.' and be_invited>0')->group('uid,coin')->fList();

            $inviteReward = array();
            foreach($inviteRewardList as $k=>$v)
            {
                $inviteReward[$v['uid']][strtoupper($v['coin'])] = Tool_Math::eftnum($v['number']);
            }
            unset($v);

            foreach ($userData as &$v)
            {
                $v['invite_count'] = $inviteUser[$v['uid']];
                $rebate = json_decode($v['rebate'], true);
                $coinCount = array();
                if($rebate)
                {
                    foreach ($rebate as $rCoin=>$rebateOne)
                    {
                        if($rCoin=='mcc_in' || $rCoin=='mcc_out')
                        {
                            continue;
                        }
                        if($rCoin=='mcc_rebate_in' || $rCoin=='mcc_rebate_out')
                        {
                            $rCoin = str_replace('rebate_', '', $rCoin);
                        }
                        $thisCoin = strtoupper(preg_replace('/(.+?)_.+/', "$1", $rCoin));
                        if(isset($coinCount[$thisCoin]))
                        {
                            $coinCount[$thisCoin] += $rebateOne;
                        }
                        else
                        {
                            $coinCount[$thisCoin] = $rebateOne;
                ...
                //奖励币总额
                $tmp = array();
                foreach ($coinCount as $coinName=>$coinNum)
                {
                    $tmp[] = sprintf('%.8f', $coinNum).$coinName;
                }
                unset($v['rebate']);

                $v['coin_count'] = implode(',', $tmp);
                $v['mo'] = preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1****$3', $v['mo']);

                $tmp = array();
                if($inviteReward[$v['uid']])
                {
                    foreach ($inviteReward[$v['uid']] as $coinName=>$coinNum)
                    {
                        $tmp[] = $coinNum.$coinName;
                    }
                }
                $v['reward'] = implode(',', $tmp);

                $thisUid = $v['uid'];
                unset($v['uid']);
                $inviteUser[$thisUid] = $v;
            }
            unset($v);
    ...
    public function accountuserAction()
    {
        ...
        }else
        {
            ...
            foreach ($coinList as $key => &$v)
            {
                ...
                if (in_array($v['name'], $coinStatus) || in_array($v['name'], array('btc', 'dob')))
                {
                    if ($bibi[$v['name'] . '_over'] > 0 || $bibi[$v['name'] . '_lock'] > 0 || $v['name'] == 'btc')
                    {
