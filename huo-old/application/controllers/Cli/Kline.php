...
class Cli_KlineController extends Ctrl_Cli
{
    ...
    public function runAction($type = '5m')
    {
        ...
        foreach ($cDatas as $v)
        {
            ...
            Cache_Redis::instance('quote')->set("{$coin}tradeline_{$type}", '{"des" : "" , "isSuc" : true  , "datas" : {"USDCNY":6.5746,"contractUnit":"' . strtoupper($coin_from) . '","data" :[' . trim(implode(',', $tJS), ',') . ']}, "marketName": "QQIBTC", "moneyType": "' . $coin_to . '", "symbol": "' . $coin . '", "url": "https://www.dobitradecoin.com"}');
            Cache_Redis::instance('quote')->set("{$coin}_btctradeline_{$type}", '{"des" : "" , "isSuc" : true  , "datas" : {"USDCNY":6.5746,"contractUnit":"' . strtoupper($coin_from) . '","data" :[' . trim(implode(',', $tJS), ',') . ']}, "marketName": "QQIBTC", "moneyType": "' . $coin_to . '", "symbol": "' . $coin . '", "url": "https://www.dobitradecoin.com"}');
        }
        exit($type);
    }
