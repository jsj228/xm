...
class Cli_WalletController extends Ctrl_Cli
{
    ...
    public function  bitAction($coin){
        ...
        if(!$coinconf || $coinconf['type']!='bit') die('币种错误');
        ...
        if (!isset($json['version']) || !$json['version']) die('钱包连接失败');
...
//        Tool_Out::p($block_info);die;
    ...
    public function btmAction($coin){
        ...
        if(!in_array($res['status'],['success','SUCCESS'])) die('获取数据失败');
        ...
        if (!$addList || count($addList)<=0) die('获取地址失败');
    ...
    public function xrpAction($coin){
        ...
        if (!$coinconf || $coinconf['type'] != 'xrp') die('币种错误');
        ...
        if ($history['result']['status'] != 'success') die('钱包连接失败');

        $lists = $history['result']['transactions'];
        if (count($lists) < 1) die('钱包无数据');
