...
class AddressModel extends Orm_Base
{
	...
	public function getAddrXrp($uid, $coin){
        ...
        if (!$label = $this->where("uid = {$uid} and coin = '{$coin}' and status = 0")->fOne('label')) {
            ...
            if (!$this->insert(['uid' => $uid, 'coin' => $coin, 'address' =>rand(1000,9999).time(), 'created' => time(),'label'=>$label])) {
                return false;
            }
        }
    ...
    public function getAddrEos($uid,$coin){
	    ...
	    if(!$label = $this->where("uid = {$uid} and coin = '{$coin}' and status = 0")->fOne('label')){
            ...
            $label = $tool->eos();
            if(!$this->insert(['uid'=>$uid,'address'=>rand(1000,9999).time(),'coin'=>$coin,'created'=>time(),'label'=>$label])){
                return false;
            }
        }
        $wallet = ['address'=>$addr,'label'=>$label];
