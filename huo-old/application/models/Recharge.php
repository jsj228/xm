...
class RechargeModel extends Orm_Base{
	...
	public $field = array(
		...
		'num' => array('type' => "float(11, 2) unsigned", 'comment' => '充值数量'),
		'mum' => array('type' => "float(11, 2) unsigned", 'comment' => '到账数量'),
		...
		'status' => array('type' => "int(4) unsigned", 'comment' => 'status'),//0 未付款 1 充值成功 2 处理中 3 已撤销
		'czr' => array('type' => "varchar(50) unsigned", 'comment' => 'czr'),//操作人
	...
	static public function getRecharge($userid='',$status=''){
		...
		$udid = $uom->fRow("select * from {$uom->table} where userid='{$userid}' and status='{$status}'");
