...
const realinfo = new Vue({
  ...
  methods: {
    ...
    //获取目前状态
    getInfo() {
      this.$http.post('/Ajax_User/userstatus', {})
      .then(({ data, status, msg }) => {
        if (status == 1 && data == '') {
          ...
          if (status == 1 || status == 2 || status == 3) {
            let { name, idcard, cardtype, frontFace, backFace, handkeep, idcardyi } = data;
            //数据填充
    ...
    uploadFile(e) {
      ...
      let trageFile = e.target.files[0];
      let name = e.target.name;
      let size = trageFile.size;
      this.lrzImg(trageFile)
      .then((result) => {
      ...
      return new Promise((resolve) => {
        if (this.submitNow) {
          ...
          if (this.status == '' && !this.failAgain) {
            ...
            resolve({ result: result, url: url, str: str });
    ...
    ajaxPost(url) {
      this.$http({ method: 'POST', url, data: this.postData, rsa: false })
      .then(({ data, status, msg }) => {
        this.submitNow = true;
        if (status == 1) {
    ...
    writeDown(errResult) {
      ...
      this.$http.post('/ajax_common/reqFailedLog', qs.stringify(errEnc, { arrayFormat: 'brackets' }), { headers });
    },
    //时间戳
    creatTimes(value) {
