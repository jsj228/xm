...
//        $this->ajax($count,0,$c2ctradeMo->getLastSql());
        ...
        // if ($count >= 5) $this->ajax("每天只能挂五笔{$txt}单",0,["repeat"=>$newRepeat]);
        ...
        if($type==2){
            ...
            $fee_price = $fee+$price;
            if ($fee_price > floatval($userInfo['cnyx_over'])) $this->ajax("您的余额不足,该比订单手续费{$fee}CNYX",0,["repeat"=>$newRepeat]);//余额不足
        ...
        try{
            ...
            if (($up_id && $in_id && $type==2) || ($type==1 && $in_id)){
                $c2ctradeMo->commit();
                $this->c2cmarket($in_id,$userBank,$newRepeat);
    ...
    //撮合买卖
    public function c2cmarket($id,$userBank,$newRepeat = "")
    {
        $this->_ajax_islogin();
        $this->ajax_validata_user();
        $c2ctradeMo = new C2ctradeModel();
        ...
        if ($order['type'] == 1) {
            ...
            $sell = $c2ctradelogMo->lock()->where($where)->fRow();
            $tagwhere = "a.status=0 and a.type=2 and a.moble!='15586991887' and a.uid!={$uid} and a.deal_id=0 and a.addtime >$atime and (a.bank={$order['bank']} or a.wechat={$order['wechat']} or a.alipay={$order['alipay']}) and ((a.min<={$order['price']} and a.deal>={$order['price']}) or a.deal={$order['price']})";
            $sell = $this->resetFrozenUser($sell,$tagwhere);
            ...
            $tagwhere = "a.status=0 and a.type=1 and a.moble!='15586991887' and a.uid!={$order['uid']} and (a.deal between $small and $big) and (a.bank={$order['bank']} or a.wechat={$order['wechat']} or a.alipay={$order['alipay']})";
            $buy = $this->resetFrozenUser($buy,$tagwhere);
     ...
     * @return array
     */
    public function resetFrozenUser($trade = array(),$where = "",$users = array()){
        if(empty($trade)){
            return $trade;
        }
        $c2ctradeMo = new C2ctradeModel();
        $userforbidden = new UserForbiddenModel();
        $userforbiddenInfo = $userforbidden->where(['uid'=>$trade['uid'],"status" => 0])->fRow();
        if(empty($userforbiddenInfo)){
            return $trade;
        }else {
            $users[] = $trade['uid'];
            if(empty($users)) {
                $tagwhere = $where." and a.uid not in (".implode(",",$users).") ";
            }
            $sql = "select a.* from c2c_trade as a left join user_forbidden as b on a.uid = b.uid and b.status != 0 where {$tagwhere} order by a.addtime asc,a.id asc FOR UPDATE";
            $trade_result = $c2ctradeMo->query($sql);
            $trade = $trade_result[0];
            return $this->resetFrozenUser($trade,$where,$users);
        }
    }
    ...
    public function confirmAction()
    {
        ...
        try {
            ...
            if ($up_sale_id && $up_buy_id && $buy_id && $sell_id && $od_id) {
                $mo->commit();
                $this->ajax('卖出成功', 1, ["repeat"=>$newRepeat]);
            } else {
                $mo->back();
                $this->ajax('卖出失败',0,["repeat"=>$newRepeat]);
            }
...
//                 var_dump($truenamesell);die;
    ...
    public function handtradeAction(){
        ...
        if ($id){
            $this->_ajax_islogin();
            if ($price < 100) {
                $this->ajax('交易的金额最少100！',0,["repeat"=>$newRepeat]);
            }

            if ($price % 100 != 0) {
                $this->ajax('交易价格必须是100的整数倍！',0,["repeat"=>$newRepeat]);
            }
            if (!is_numeric($price) || strpos($price, ".") !== false) {
                $this->ajax('交易价格必须是100的正整数的倍数！',0,["repeat"=>$newRepeat]);
            }
            ...
            if ($type == 1){
                ...
                if (!$bank) {
                    $this->ajax('请绑定付款方式，是否开启状态！',0,["repeat"=>$newRepeat]);
                }

                $sell = $C2ctrade->where("id =$id and deal_id=0 and status=0")->fRow();
                if (!$sell){
                    $this->ajax('订单已匹配，请选择其他订单交易',0,["repeat"=>$newRepeat]);
                ...
                if ($sy_num - $min_num >= 0){
                    if ($price - $min_num < 0){
                        $this->ajax('交易数量不得低于最小匹配数量',0,["repeat"=>$newRepeat]);

                    }
                    if ($price > $sy_num){
                        $this->ajax('交易数量大于卖方的剩余可交易数量，无法交易',0,["repeat"=>$newRepeat]);
                ...
                if ($price < $buy['price']){
                    $this->ajax('交易数量不可以小于买方挂单数量',0,["repeat"=>$newRepeat]);
                    $mo->back();
                }
                if ($price > $buy['price']){
                    $this->ajax('交易超过买方挂单数量',0,["repeat"=>$newRepeat]);
                    $mo->back();
                }
                ...
                if ($mo){
                    ...
                    $mo->back();
                    $this->ajax('下单失败！',0,["repeat"=>$newRepeat]);
                }
            }else{
                $this->ajax('交易类型不存在',0,["repeat"=>$newRepeat]);
            }
        }
